...
abstract class AbstractDispatcherLeaderProcess implements DispatcherLeaderProcess {
	...
	protected <V> Optional<V> supplyIfRunning(Supplier<V> supplier) {
		synchronized (lock) {
			if (state != State.RUNNING) {
				return Optional.empty();
			}
	...
	protected void runIfStateIs(State expectedState, Runnable action) {
		runIfState(expectedState::equals, action);
	}

	protected void runIfStateIsNot(State notExpectedState, Runnable action) {
		runIfState(
			state -> !notExpectedState.equals(state),
			action);
	}

	protected void runIfState(Predicate<State> actionPredicate, Runnable action) {
		synchronized (lock) {
	...
	protected <T> Void onErrorIfRunning(T ignored, Throwable throwable) {
		synchronized (lock) {
			if (state != State.RUNNING) {
				return null;
			}
