		...
		FutureUtils.assertNoException(
			jobManagerRunner.getResultFuture().handleAsync(
				(ArchivedExecutionGraph archivedExecutionGraph, Throwable throwable) -> {
					...
					final Optional<CompletableFuture<JobManagerRunner>> optionalJobManagerRunner = Optional.ofNullable(jobManagerRunnerFutures.get(jobId));
					final JobManagerRunner currentJobManagerRunner = optionalJobManagerRunner.map(future -> future.getNow(null)).orElse(null);
